...
// src/middleware/check-session-id-exit.ts
async function checkSessionIdExit(request, reply) {
...
async function transactionsRoutes(app2) {
  app2.get("/", { preHandler: [checkSessionIdExit] }, async (request) => {
    const { sessionId } = request.cookies;
    const transactions = await knex("transactions").where("session_id", sessionId).select();
    return { transactions };
  });
  app2.get("/:id", { preHandler: [checkSessionIdExit] }, async (request) => {
  ...
  app2.get("/summary", { preHandler: [checkSessionIdExit] }, async (request) => {
    const { sessionId } = request.cookies;
